...
function DawnTransition({ onCommit, onFinish }: { onCommit: () => void; onFinish: () => void }) {
  ...
  return (
    <div className="theme-overlay dawn-overlay" aria-hidden="true">
      <div
        className="dawn-light"
        style={{ '--light-scale': lightScale } as React.CSSProperties}
      />
      {showParticles && particles.map(p => (
        <div
          key={p.id}
          className="dawn-particle"
          style={{
            left: p.left,
            top: p.top,
            width: p.size,
...
/* ── Global night fireflies ────────────────────────────────── */
export function GlobalNightEffects() {
  const { pendingMode } = useTheme()
  ...
  return (
    <div
      className={`global-night-effects${pendingMode === 'day' ? ' night-effects-out' : ''}`}
      aria-hidden="true"
    >
      {fireflies.map(f => (
        <div
